...
vector<int> get_solution_provide_indicator(
...
vector<vector<int>> latent_truth_model(
	...
	for (int question_id = 0; question_id < question_num; question_id++) {
		...
		vector<int> sol_provide_ind = get_solution_provide_indicator(observations);
...
vector<WordVec> kmeans_init(vector<Keyword> &keywords, WordModel &word_model,
	...
	for (int i = 1; i < cluster_num; i++) {
		...
		for (int j = 0; j < keywords.size(); j++) {
			...
			int kv_size = kv.size();
			int cv_size = cv.size();
			if(kv.size()!=cv.size() || kv.size()!=word_model.dimension) throw "wrong size";
