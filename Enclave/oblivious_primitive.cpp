...
inline void oblivious_assign_score_pair(uint8_t pred, pair<double, int> &dst,
		pair<double, int> &t, pair<double, int> &f) {
	dst.second = oblivious_assign_CMOV(pred, t.second, f.second);
	double tmp_first = oblivious_assign_CMOV(pred, t.first * SCORE_SCALE_FACTOR,
			f.first * SCORE_SCALE_FACTOR);
	dst.first = tmp_first / SCORE_SCALE_FACTOR;
}

void oblivious_compare_and_swap(pair<double, int> &a, pair<double, int> &b,
		int sort_direction,
...
void __oblivious_bitonic_sort(vector<pair<double, int>> &arr, int start_index,
		int end_index, int sort_direction,
		function<bool(pair<double, int> &a, pair<double, int> &b)> &cmp) {
...
#pragma omp parallel for
		for (int k = start_index; k < end_index; k += j) {
			for (int i = k; i < k + j / 2; i++) {
				oblivious_compare_and_swap(arr[i], arr[j / 2 + i],
						sort_direction, cmp);
			}
		}
	}
...
void oblivious_bitonic_sort(vector<pair<double, int>> &arr, int sort_direction,
	...
	pair<double, int> dummy = arr[myRand::myrand_int() % arr.size()];
	dummy.second = -1;
...
//    cout<<arr.size()<<endl;
	for (int j = 2; j <= arr.size(); j *= 2) {
		for (int i = 0; i < arr.size(); i += j) { // can use openmp accelerate this part
			__oblivious_bitonic_sort(arr, i, i + j,
					(i / j % 2) ^ sort_direction, cmp);
		}
	}
...
string oblivious_assign_string(uint8_t pred, const string &a, const string &b) {
	...
	for (int i = 0; i < a.size(); i += 4) {
		*(uint32_t*) resp = oblivious_assign_CMOV(pred, *(uint32_t*) ap,
				*(uint32_t*) bp);
		ap += 4;
		bp += 4;
		resp += 4;
...
void oblivious_assign_keyword(uint8_t pred, Keyword &dst, Keyword &t,
		Keyword &f, bool pure) {
	...
	dst.cluster_assignment = oblivious_assign_CMOV(pred, t.cluster_assignment,
			f.cluster_assignment);
...
void oblivious_compare_and_swap(Keyword &a, Keyword &b, int sort_direction,
	...
	if (a.question_id == -1 && b.question_id == -1)
		return;
	if (a.question_id == -1) {
		if (sort_direction == 0)
			std::swap(a, b);
...
//
		if (pure) {
			...
			Keyword tmp;
			tmp.shuffle_tag = a.shuffle_tag;
			tmp.question_id = a.question_id;
			tmp.cluster_assignment = a.cluster_assignment;
			tmp.owner_id = a.owner_id;
			oblivious_assign_keyword(flag, a, b, a, pure);
			oblivious_assign_keyword(flag, b, tmp, b, pure);
			// only swap pointer is enough for our case
			// simulate swap pointer
			if (flag) {
				std::swap(a.content, b.content);
			}
		}
	}
...
void __oblivious_bitonic_sort(vector<Keyword> &arr, int start_index,
		int end_index, int sort_direction,
		function<bool(Keyword&, Keyword&)> cmp, bool pure) {
	int ele = end_index - start_index;
	for (int j = ele; j >= 2; j /= 2) {
		for (int k = start_index; k < end_index; k += j) {
			for (int i = k; i < k + j / 2; i++) {
				oblivious_compare_and_swap(arr[i], arr[j / 2 + i],
						sort_direction, cmp, pure);
			}
		}
	}
}

void oblivious_bitonic_sort(vector<Keyword> &arr, int sort_direction,
		function<bool(Keyword&, Keyword&)> &cmp, bool pure) {
	// pad with dummy to fit 2^n;
	int level = log2(arr.size());
	if (pow(2, level) != arr.size()) {
...
//    cout<<arr.size()<<endl;
	for (int j = 2; j <= arr.size(); j *= 2) {
		...
		for (int i = 0; i < arr.size(); i += j) { // can use openmp accelerate this part
			__oblivious_bitonic_sort(arr, i, i + j,
					(i / j % 2) ^ sort_direction, cmp, pure);
		}
	}

	//remove dummy
	for (auto &keyword : arr) {
		if (keyword.question_id == -1)
			continue;
...
void oblivious_shuffle(vector<Keyword> &arr) {
	...
	for (auto &keyword : arr) {
		keyword.shuffle_tag = myRand::myrand_int();
	}
...
void keywords_padding(vector<Keyword> &keywords) {
	...
	for (auto &keyword : keywords) {
		...
		if (length > max_keyword_length)
			max_keyword_length = length;
	}
	// set to the multiply of 4
	if (max_keyword_length % KEYWORD_LENGTH_ALIGN != 0)
		max_keyword_length = (max_keyword_length / KEYWORD_LENGTH_ALIGN + 1)
				* KEYWORD_LENGTH_ALIGN;
	// pad all keywords to max_keyword_length
	for (auto &keyword : keywords) {
		keyword.content += string(max_keyword_length - keyword.content.size(),
				KEYWORD_PAD_CHAR);
	}
}
...
vector<Keyword> oblivious_vocabulary_decide(vector<Keyword> &keywords) {
	...
	for (int i = 1; i < keywords.size(); i++) {
		int tag = oblivious_assign_CMOV(
				keywords[i].content != keywords[i - 1].content, FIRST_OCCUR,
				NON_FIRST_OCCUR);
		keywords[i].shuffle_tag = tag;

	}
	//ocall_print_string(("test count:"+to_string(count)+"\n").c_str());
	oblivious_bitonic_sort2(keywords, 0, kw_shuffle_cmp,true);
...
int sample_gamma(double p, int alpha) {
	const double base_prob = 0.5 * (1 - p / 2) * pow(1 - p, alpha);

	double pr = (double) myRand::myrandom();
	while (pr < base_prob || pr > 1 - base_prob) {
		pr = (double) myRand::myrandom();
	}
	int k = 0;
	// k<0
	...
	if (gamma < 0 || gamma > 2 * alpha) {
		throw "gamma out of range";
	}
	return gamma;
}

double oblivious_dummy_words_addition(vector<Keyword> &padded_vocabulary,
		vector<Keyword> &keywords, float epsilon, float delta) {
	const int DUMMY_TAG = -1;
	const int DUMMY_DUMMY_TAG = 1;

//    const float epsilon = 3;
//    const float delta = -32; // 2^(-32);
	double p = 1 - exp(-epsilon);
	double tmp = (delta - log2(0.5 - p / 4) - log2(padded_vocabulary.size()))
			/ log2(1 - p);
	int alpha = tmp;
	if (tmp - alpha > 1e-8)
		alpha += 1;
	...
	for (int i = 0; i < keywords.size(); i++) {
		//ocall_print_string((keywords[i].content+"\n").c_str());
	...
	for (int i = 0; i < padded_vocabulary.size(); ++i) {
		...
		for (int j = 0; j < 2 * alpha; j++) {
			dummy.owner_id = -1;
			dummy.shuffle_tag = oblivious_assign_CMOV(j < gamma, DUMMY_TAG,
					DUMMY_DUMMY_TAG);
	...
	// oblivious sort
	function<bool(Keyword&, Keyword&)> kw_shuffle_cmp(keyword_shuffle_compare);
	oblivious_bitonic_sort2(keywords, 0, kw_shuffle_cmp);

	int real_count = 0;
	vector<Keyword> tmp_arr;
	for (int i = 0; i < keywords.size(); i++) {
		if (keywords[i].shuffle_tag != DUMMY_TAG)
			break;
		tmp_arr.push_back(std::move(keywords[i]));
		//ocall_print_string((tmp_arr[i].content+"\n").c_str());
...
void oblivious_assign_keyword2(uint8_t pred, Keyword &dst, Keyword &t,
		Keyword &f, bool pure) {
	dst.shuffle_tag = oblivious_assign_CMOV(pred, t.shuffle_tag, f.shuffle_tag);
	dst.question_id = oblivious_assign_CMOV(pred, t.question_id, f.question_id);
	dst.cluster_assignment = oblivious_assign_CMOV(pred, t.cluster_assignment,
...
void oblivious_compare_and_swap2(Keyword &a, Keyword &b, int sort_direction,
		function<bool(Keyword&, Keyword&)> &cmp, bool pure) {
	// 0 ? a>b: a<b
	// not need to oblivious swap padding words for bitonic sort, assume the padded elements are always biggest, or smallest
	bool flag;
...
void bitonicMerge(vector<Keyword> &arr, int start_index, int end_index,
		int sort_direction, function<bool(Keyword&, Keyword&)> cmp, bool pure) {
...
void __oblivious_bitonic_sort2(vector<Keyword> &arr, int start_index,
		int end_index, int sort_direction,
		function<bool(Keyword&, Keyword&)> cmp, bool pure) {
	...
	if (n > 1) {
		int m = n / 2;
		__oblivious_bitonic_sort2(arr, start_index, m + start_index,
				!sort_direction, cmp, pure);
		__oblivious_bitonic_sort2(arr, start_index + m, end_index,
				sort_direction, cmp, pure);
		bitonicMerge(arr, start_index, end_index, sort_direction, cmp, pure);
	}
}

void oblivious_bitonic_sort2(vector<Keyword> &arr, int sort_direction,
		function<bool(Keyword&, Keyword&)> &cmp, bool pure) {
	__oblivious_bitonic_sort2(arr, 0, arr.size(), !sort_direction, cmp, pure);
}
